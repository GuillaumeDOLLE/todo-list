const taskManager = {
    ...
    handleCreateForm: async function (event) {
        ...
        try {
            ...
            const createdTaskDataObject = await response.json();
            ...
            taskManager.insertTaskInHtml(createdTaskDataObject);
        
            // BONUS: réinitialiser le formulaire après l'ajout d'une tâche
            event.target.reset();
    ...
    handleDeleteButton: async function (event) {
        ...
        try {
            ...
            if(response.status !== 204) {
                ...
                throw new Error(`Impossible de supprimer cette tâche. Statut: ${response.status}`);
            }

            // On supprime l'élément dans la page HTML
            taskHtmlElement.remove();
            // taskHtmlElement.addEventListener('click', function(e) {
            //     Bulma().alert({
            //         type: 'info',
            //         title: 'Ceci est une notification !',
            //         body: 'La tâche a bien été supprimée',
            //         confirm: 'Confirmer',
            //         cancel: 'Annuler'
            //     });
            // });
    ...
    handleEditForm: async function (event) {
        ...
        try {
            ...
            if(response.status !== 200) {
                throw new Error(`Impossible d'éditer le nom de cette tâche. Status: ${response.status}`);
            }
            
            // Après confirmation de l'API modifier le nom de la tâche dans le span.task__name
            const newTaskName = taskFormData.get('name');
            
            const spanTaskNameHTMLElm = taskHtmlElement.querySelector('.task__name');
            spanTaskNameHTMLElm.textContent = newTaskName;
    ...
    // insertNotificationInHTML: function () {
    //     const notifTemplate = document.querySelector('.template-notification');
    //     console.log(notifTemplate);
    //     const newNotif = document.importNode(notifTemplate.content, true);
    //     document.querySelector('.notifications').append(newNotif);
    // }
